 ...
 */
public class ScreenCaptureContentObserver extends ContentObserver {
    private static final String TAG = "ScreenCaptureContentObserver";
    ...
    public ScreenCaptureContentObserver(Context context, int width, int height){
        super(null);
        mScreenWidth = width;
        ...
        if(Build.VERSION.SDK_INT < 23) {
            ...
            //判断厂商，设置监听目录
            if(android.os.Build.MANUFACTURER.equalsIgnoreCase(CommonBadgeUtilImpl.MANUFACTURER_OF_HARDWARE_XIAOMI)){
                screenshotPath = ScreenShotPath.XIAOMI;
            }else if(android.os.Build.MANUFACTURER.equalsIgnoreCase(CommonBadgeUtilImpl.MANUFACTURER_OF_HARDWARE_VIVO)){
        ...
        if (uri == null) { //API16以下版本
            try {
                ...
                if (cursor != null && cursor.getCount() > 0) {
                    ...
                    //加个过滤条件必须是1.5S内的图片，且路径中包含截图字样“screenshot”
    ...
    /**
     * 检查文件添加时间，添加时间与当前时间不超过1.5s，大部分时候不超过1s。
